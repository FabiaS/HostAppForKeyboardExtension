...
class NavigationBarView: UIView {
    ...
    func nextKeyboardButtonPressed() {
        delegate?.nextKeyboardPressed(sender: self)
    }
    
    func pagePlusOneButtonPressed() {
        delegate?.pagePlusOnePressed(sender: self)
    }
    
    func pageMinusOneButtonPressed() {
    ...
    func movePageControlUp(_ step: Int) {
    ...
    func movePageControlDown(_ step: Int) {
        let newPageNumber = pageControl.currentPage-step
        if newPageNumber >= 0 {
            pageControl.currentPage = newPageNumber
        }
    ...
    func updatePageControlWith(scrollViewContentOffset: CGPoint, scrollViewPageSize: CGSize) {
